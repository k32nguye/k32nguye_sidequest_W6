...
export async function loadAssets(levelPkg, tuningDoc) {
  ...
  const playerImg = await loadImageAsync("assets/hunterSpriteSheet.png");
  const boarImg = await loadImageAsync("assets/hyenaSpriteSheet.png");
  const leafImg = await loadImageAsync("assets/hareIdle.png");
...
// determine properties by looking at file, adjust frameSize based on calculated frame size:
// frame width and height = 288 divided by 6 columns (6 rows) = 48px per frame
